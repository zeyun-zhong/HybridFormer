...
class Feeder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # data_numpy = np.array(data_numpy)  # Zeyun: no need, since data is already in numpy array format
        # valid_frame_num = np.sum(data_numpy.sum(0).sum(-1).sum(-1) != 0)
        valid_frame_num = np.count_nonzero(np.any(data_numpy, axis=(0, -2, -1)))  # Zeyun: a more efficient implementation
    ...
    def top_k(self, score, top_k, return_acc=True):
        rank = score.argsort()
